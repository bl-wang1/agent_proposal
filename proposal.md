...
版本：2026-06-28  
方向：以空间世界模型为主线，融合主动空间感知、任务规划、技能执行与受控学习  
...
我建议研究 ECAOS：一个面向长程具身任务的空间感知 Agent 框架。它把高层 Agent 规划、空间世界模型、主动感知策略、标准化技能接口、空间谓词验证器、VLA/导航/操作控制器和安全治理组合成一个可实验、可迭代的系统。研究目标不是再训练一个单一“大模型”，而是回答：如何让机器人把真实环境编码为可行动的空间状态，并在不确定、遮挡、关系冲突或可达性未知时主动补充感知，从而更可靠地完成长程导航与操作任务。

本 proposal 收敛到四个核心机制：`SpatialFact Graph` 负责空间信息编码，`Active Perception Policy` 负责主动补齐不确定事实，`Spatial Verifier` 负责验证空间 effect 是否达成，`Skill Contract` 负责把导航、操作和 VLA 技能接入同一套 precondition/effect/verifier 协议。

更精确地说，ECAOS 的研究命题不是“把很多模块拼成 AgentOS”，而是：

> 在长程具身任务中，能否通过结构化空间世界模型、主动空间感知和空间谓词验证，把真实环境中的对象、关系、可达性、遮挡与 affordance 编码进 Agent 决策，从而减少空间误判、隐藏失败和重复探索，并提升任务成功率？

这个命题可被反驳。如果空间世界模型不能比文本上下文或普通地图更好地支持任务规划，如果主动感知不能降低关系误判、遮挡失败或可达性错误，如果空间谓词验证不能比技能返回值更早发现隐藏失败，那么 ECAOS 的核心假设就不成立。
...
3. 主动学习是空间感知的一部分；当空间事实低置信度、互相冲突或影响高风险动作时，系统应主动 inspect、换视角、低风险探测或请求人类标注。
4. 持续学习、安全治理和版本准入是支撑模块；它们服务于空间事实、感知器、affordance 估计和策略更新，而不是本 proposal 的主贡献。

核心贡献边界：

1. 不是提出新的低层 VLA 模型，而是定义 VLA/导航/操作策略如何作为可验证 skill 被 Agent runtime 调度。
2. 不是只做机器人 lifelong fine-tuning，而是把空间失败变成可诊断的世界模型更新、感知器更新和主动学习样本。
3. 不是只做数字 Agent 框架迁移，而是把物理世界的空间谓词、precondition、effect、safety gate、rollback 和 verifier 纳入 agent loop。
4. 不是只用 3D memory 提供上下文，而是让 memory 参与不确定性估计、主动感知、失败归因和技能选择。
...
设计并验证一个以空间世界模型为核心的具身 Agent 框架，使机器人能在动态真实环境中编码、感知、验证和主动更新空间信息，并用这些空间状态完成长程操作与导航任务。

### 2.2 具体目标

1. 构建一个空间世界模型，把拓扑、语义对象、局部几何、可达性、遮挡、affordance、时间戳和置信度编码为可查询状态。
...
第一版 ECAOS 只证明四个机制是否成立：

1. `SpatialFact Graph`：把真实空间状态写成可查询、可过期、可追溯的空间事实。
2. `Active Perception Policy`：当空间事实低置信度、冲突、遮挡、可达性未知或影响高风险动作时，选择 `inspect`、`change_viewpoint`、`probe_reachability`、`ask_human` 或 `defer`。
3. `Spatial Verifier`：检查技能执行后的空间谓词是否成立，而不是相信裸 `success` 返回。
4. `Skill Contract`：每个技能都声明 `precondition`、`effect`、`verifier`、`rollback`、`risk` 和 `failure_code`。
...
低置信度、过期或高风险相关的 `SpatialFact` 不能直接驱动执行，只能触发主动感知、人工确认或延迟动作。
...
因此，ECAOS 的创新不在单点模型，而在“以空间世界模型为核心的具身 Agent 框架”：空间信息被结构化编码，Agent 主动补齐不确定空间事实，并用空间谓词验证任务是否真的推进。
...
2. Active Perception Policy：当空间事实不确定、互相冲突或影响高风险动作时，Agent 主动选择 inspect、换视角、低风险探测或请求人类标注。
3. Spatial Verifier：verifier 检查 visible、reachable、on/inside/near 等空间 effect 是否真的发生，专门发现 hidden spatial failure。
4. Skill Contract：skill 不是普通 API，而是 intent、contract、implementation、verifier 四元组；contract 明确 precondition、effect、rollback、risk 和 failure code。

如果这四点不能在实验中带来可测收益，ECAOS 就不应被视作独立研究贡献。持续学习、candidate promotion 和安全治理仍然保留，但它们是空间事实和感知能力更新的准入机制，而不是主命题本身。
...
2. Spatial World Model Layer：维护拓扑、语义对象、局部几何、遮挡、可达性、affordance、时间戳和置信度。
...
低置信度或过期的 SpatialFact 不能直接驱动高风险动作，只能触发 `inspect`、`change_viewpoint`、`probe_reachability`、`ask_human` 或低风险探索。这样可以显式处理“错误记忆比没有记忆更危险”的问题。
...
### 6.6.1 Failure-to-Spatial-Learning Pipeline
...
### 6.7.1 Trace-Level Credit Assignment
...
#### 6.7.2 Agentic RL transition builder
...
### 6.9 Candidate Promotion Policy
...
4. 实现 planner、spatial verifier、world model、active perception stub、governance 的最小闭环。
...
| Week 4-6 | planner、governance、spatial world snapshot、verifier 最小闭环 | text-memory、2D-map、ECAOS-spatial-lite 三个 baseline | 能统计 spatial grounding、relation accuracy、hidden spatial failure、human label burden |
...
| Week 11-13 | shadow evaluation、量化门槛、报告面板、关键案例回放 | 固定 split 的首轮消融实验 | 证明 spatial world model、active perception 和 verifier 是否带来显著增益；若没有，定位失败模块 |

### 阶段 1：空间世界模型与长程规划

目标：验证结构化空间世界模型对长程成功率和空间错误率的增益。
...
### 阶段 3：Agentic RL 优化规划与恢复

目标：用 RL 优化高层 agent 决策，而不是只靠 prompt 或手写规则。
...
Main variable: spatial world model + active perception + spatial predicate verifier
```
...
4. ECAOS-spatial-lite：空间世界模型 + 空间谓词 verifier。
...
最小实验只需要证明一个硬命题：结构化空间世界模型 + 主动空间感知 + 空间谓词 verifier 能比文本上下文、2D map 或静态 3D semantic memory 更少空间误判、更早发现隐藏空间失败，并提升长程任务成功率。真实机器人和跨本体可以作为第二阶段。
...
1. 一个以空间世界模型为核心的具身 Agent 框架，把拓扑、对象、几何、可达性、遮挡、affordance 和时间状态接入任务规划。
2. 一个主动空间感知机制，在不确定、遮挡、关系冲突和高风险动作前选择 inspect、换视角、低风险探测或请求标注。
3. 一个标准化 embodied skill interface，连接 VLA、导航、操作、经典控制和工具调用，并用空间谓词验证 effect。
4. 一个 trace-to-spatial-update 方案，把失败轨迹转成候选 SpatialFact、relation checker、reachability predictor 和 active perception policy。
5. 一个安全治理框架，把低置信度空间事实、高风险动作、human checkpoint 和 audit trail 外置。
6. 一套可复现实验矩阵，覆盖空间编码消融、主动感知消融、空间验证和受控真实 shadow。
...
### 第 2 轮：从“记忆库”转向“空间世界模型”

普通向量记忆不足以支持具身任务。机器人需要知道物体在哪里、是否可见、是否可达、哪里被遮挡、哪个表面可放置、哪个技能对哪个对象有效。因此 ECAOS 把 memory 升级为空间世界模型：拓扑、对象、几何、可达性、affordance、时间和置信度共同构成 Agent 状态。
...
> 在长程家居操作与导航任务中，一个用空间世界模型、主动空间感知和空间谓词验证驱动的 embodied Agent 框架，是否能比文本记忆、2D map 和静态 3D semantic memory，在空间 grounding、关系/可达性判断、隐藏空间失败率和长程成功率上显著更好？
